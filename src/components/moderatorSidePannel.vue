<template>
    <div v-if="props.tileData" :key="props.tileData.id">
        ...
        <ul>
            <li v-for="group in groups" :key="group.id + tileData.id"
                :class="{ checkThis: group.verify.includes(tileData.id) }"><label :for="group.id + tileData.id">
                    {{ group.name }} : {{
                        group.verify.includes(tileData.id) }}<br>
                        collected: <input :id="group.id + tileData.id" type="checkbox" :key="group.id + tileData.id"
